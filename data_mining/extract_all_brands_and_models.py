...
for brand in all_car_brands:
    if brand in BRANDS_TO_LOOK_FOR:
        ...
        model_to_loop = [model.text for model in new_soup.find("select", {"name": "model"}).findChildren("option", recursive=False) if model.text in SKODA_MODELS]
        ...
        for model in model_to_loop:
            ...
            print(df)
            df.to_excel(f"car_brands_and_models_links_Skoda.xlsx")
    time.sleep(2)
